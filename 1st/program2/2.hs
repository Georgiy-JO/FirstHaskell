...
p1 :: Person
p1 = Person "Jack ONeill" 42 65.5
p2 :: Person
p2=Person "Jim Hot Keys" 19 61
...
p3 :: Person2
p3=Pr2 {
    name2 = "Kim the Possible",
    age2 = 18,
    weight2 = 46.2
}
p4 :: Person2
p4=Pr2 {
    name2 = "Grand Karcist Ion",
    age2 = 3820,
    weight2 = 68.9
}
...
{-There will be NO read for wrrite2. I can create it if it is needed -}
...
{-THAT IS THE DIFFERENCE FROM THE "PROGRAM#1"-->-}
{- functions that return only numbers from a line (in string type) -}
numsf2 str = foldl (\(ys) x -> if ((x<='9') && (x>='0') || (x==' ') || (x=='.')) then (ys) ++ [x]  else (ys)) [] str
numsf1 (x:xs)=if(x==':')
                then numsf2 (xs)
                else numsf1 (xs)
numsf (x:xs) = numsf1 (xs)
{-<--THAT IS THE DIFFERENCE FROM THE "PROGRAM#1"-}
